...
public class AccountLoader : GLib.Object {
...
/*                } catch (Error err) {
    ...
    public static async void store_to_file(Geary.AccountInformation info, Cancellable? cancellable = null) {
        ...
        key_file.set_value(Geary.Config.GROUP, Geary.Config.IMAP_USERNAME_KEY, info.imap.credentials.user);
        key_file.set_boolean(Geary.Config.GROUP, Geary.Config.IMAP_REMEMBER_PASSWORD_KEY, info.imap.remember_password);
        if (info.smtp.credentials != null)
            key_file.set_value(Geary.Config.GROUP, Geary.Config.SMTP_USERNAME_KEY, info.smtp.credentials.user);
        key_file.set_boolean(Geary.Config.GROUP, Geary.Config.SMTP_REMEMBER_PASSWORD_KEY, info.smtp.remember_password);
        ...
        if (info.service_provider == Geary.ServiceProvider.OTHER) {
            info.imap.save_settings(ref key_file);
            info.smtp.save_settings(ref key_file);
        }
